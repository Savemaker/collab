...
//	if (access(cmnd[0], X_OK) == -1) // балииин!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!ЗАТЕСТ
//		perror("minishell");
//	if ((lstat(cmnd[0], &s)) == -1) // балииин 2!!!!!!!!!!!!!!!!!!!!!!!!!!!
//		perror("minishell");
...
char		*check_extern_command(char **cmnd)
{
	...
	i = 0;
	paths_quant = 0;
	env_path = getenv_cmnd("PATH"); //ok
	paths = ft_strsplit(env_path, ':'); //ok
