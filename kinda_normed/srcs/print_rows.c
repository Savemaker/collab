...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/11 18:44:21 by bellyn-t          #+#    #+#             */
/*   Updated: 2019/08/11 18:48:10 by bellyn-t         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		get_rowstruct_info(t_dir *object, t_row_tty *row_struct)
{
	...
	if ((strlen = ft_strlen(
			getpwuid(object->uid)->pw_name)) > row_struct->uid_len)
		row_struct->uid_len = (uint16_t)strlen;
	if ((strlen = ft_strlen(
			getgrgid(object->gid)->gr_name)) > row_struct->gid_len)
	...
	if (listxattr(object->path, row_struct->xattr, ATTR_NAMESTR, XATTR_NOFOLLOW)
		|| acl_get_link_np(object->path, ACL_TYPE_EXTENDED))
