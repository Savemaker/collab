...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/17 21:15:25 by bellyn-t          #+#    #+#             */
/*   Updated: 2019/08/31 14:07:34 by bellyn-t         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_col_row(t_dir *dir, int16_t flags, struct winsize window)
{
	if (check_flags(flags, 'l') || check_flags(flags, 'g'))
...
void	print(t_dir *dir)
{
	...
	{
		print_col_row(dir, dir->flags, window);
		exit(0);
	}
	...
	while (object)
	{
		...
		if (object->inside_files)
		{
			...
			if ((check_flags(flags, 'R') && (ft_strcmp(object->name, ".")) &&
				(ft_strcmp(object->name, ".."))) || ((check_flags(flags, 'R')
					&& (object->level) == 1)))
