...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/17 21:43:09 by bellyn-t          #+#    #+#             */
/*   Updated: 2019/08/31 17:43:29 by bellyn-t         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			print_access(t_dir *object, char *buf, t_row_tty *row_struct)
{
	...
	(row_struct->xattr_lstsize = listxattr(object->path,
	row_struct->xattr, ATTR_NAMESTR, XATTR_NOFOLLOW) > 0))
		buf[i++] = '@';
	else if (define_mode(object->mode) != 'b' &&
	define_mode(object->mode) != 'c' &&
...
int			print_name(t_dir *object, char *buf)
{
	...
	&& (define_mode(object->mode) != 'b') && (define_mode(object->mode) != 'p'))
		i += reset_color(buf + i, 0);
	if ((S_ISVTX & object->mode) != S_ISVTX)
		i += reset_color(buf + i, 0);
	if (define_mode(object->mode) == 'c')
		i += reset_color(buf + i, 2);
	else
